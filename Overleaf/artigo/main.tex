...
\begin{resumo}
  A inspeção de anomalias superficiais em chapas de rochas ornamentais é majoritariamente manual, subjetiva e de difícil padronização. Este trabalho propõe o ARIA (Análise e Reconhecimento Inteligente de Anomalias), um \textit{pipeline} hierárquico de visão computacional: uma classificação taxonômica prévia (Xception) restringe o domínio visual de um segmentador \textit{Teacher-Student}, no qual o \textit{Segment Anything Model} (SAM) gera anotações calibradas (\textit{pseudo-labels}) que treinam modelos especialistas YOLO11-seg. A hipótese é que essa especialização reduz falsos positivos frente a modelos generalistas monolíticos. Os resultados iniciais, na litologia \textit{ice leke}, confirmam que o modelo de fundação segmenta anomalias em regime \textit{zero-shot} quando guiado por \textit{prompts} calibrados por litologia.
