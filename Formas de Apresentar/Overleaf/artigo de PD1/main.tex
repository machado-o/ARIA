...
\author{Henrique Machado de Oliveira\inst{1}, Everson Scherrer Borges\inst{1}}

\address{Instituto Federal do Espírito Santo (IFES) -- Campus Cachoeiro de
  Itapemirim\\
  Espírito Santo -- Brasil
  \email{rikemachado999@gmail.com, eversonborges@gmail.com}
}
...
\begin{abstract}
  Surface quality control of ornamental stone slabs is still largely manual, subjective and hard to standardize. This work proposes ARIA, a hierarchical computer-vision pipeline in which a prior taxonomic classification (Xception) restricts the visual domain of a \textit{Teacher-Student} segmenter: the Segment Anything Model (SAM) generates calibrated annotations (pseudo-labels) that train specialist YOLO11-seg models. We hypothesize that this specialization reduces false positives compared to monolithic generalist models. Initial results on the \textit{ice leke} lithotype confirm that the foundation model segments relevant anomalies in a zero-shot setting when guided by lithology-calibrated prompts.

\end{abstract}

\begin{resumo}
  A inspeção de anomalias superficiais em chapas de rochas ornamentais é majoritariamente manual, subjetiva e de difícil padronização. Este trabalho propõe o ARIA (Análise e Reconhecimento Inteligente de Anomalias), um \textit{pipeline} hierárquico no qual uma classificação taxonômica prévia (Xception) restringe o domínio visual de um segmentador \textit{Teacher-Student}: o \textit{Segment Anything Model} (SAM) gera anotações calibradas (\textit{pseudo-labels}) que treinam modelos especialistas YOLO11-seg. A hipótese é que essa especialização reduz falsos positivos frente a modelos generalistas. Os resultados iniciais confirmam que o SAM segmenta anomalias em regime \textit{zero-shot} quando guiado por \textit{prompts} calibrados por litologia.
\end{resumo}

\section{Introdução}

O controle de qualidade de chapas é uma etapa crítica da indústria de rochas ornamentais, setor estratégico para as exportações brasileiras e fortemente concentrado no Espírito Santo, principal polo produtor e exportador do país~\cite{stone_economy_bahiense2023}. Nessa etapa, a identificação de anomalias superficiais --- como fissuras, veios, manchas e oxidações, entre outras --- determina a classificação da chapa e, por consequência, o seu valor comercial final. A tarefa é realizada de forma majoritariamente manual, dependendo da percepção visual do operador~\cite{stone_inspection_vidal2013}.

Essa inspeção manual carrega limitações estruturais. Não existe um critério único e padronizado para definir o que constitui uma anomalia, de modo que operadores diferentes avaliam a mesma chapa de maneiras distintas. A própria experiência do avaliador atua como variável: à medida que realiza repetidamente a marcação, sua forma de enxergar as feições se altera, somando-se a isso a fadiga ao longo da jornada e a natureza repetitiva da atividade. O resultado é um processo inerentemente subjetivo, inconsistente entre avaliadores e de difícil padronização~\cite{stone_subjectivity_castro2018}. Como a classificação derivada dessa inspeção define o preço final do produto, sua inconsistência traduz-se diretamente em precificação imprecisa e em perdas industriais.

A automação dessa inspeção por meio de visão computacional esbarra em dois obstáculos. O primeiro é a alta variabilidade geológica das rochas: uma feição que constitui anomalia comercial em uma litologia (uma fissura em mármore branco) pode ser um padrão natural esperado em outra (um veio mineral em granito). Modelos generalistas monolíticos, treinados sobre todas as litologias ao mesmo tempo, tendem a confundir essas feições e a gerar falsos positivos. O segundo é o custo da anotação: a rotulagem manual de milhares de imagens com defeitos sutis é custosa e limitante para a adoção industrial.

Este trabalho propõe o ARIA (Análise e Reconhecimento Inteligente de Anomalias), um \textit{pipeline} hierárquico de Inteligência Artificial concebido para ser o núcleo inteligente da plataforma industrial Hartheus, voltada ao controle de qualidade de chapas. A proposta articula três estágios: um classificador taxonômico baseado na arquitetura Xception --- o DeepStoneAI, rede convolucional previamente desenvolvida e publicada pelo grupo~\cite{deepstoneai_moreira2025} ---, que identifica o domínio litológico da chapa; o \textit{Segment Anything Model} (SAM)~\cite{sam_kirillov2023}, que atua como Professor em uma arquitetura \textit{Teacher-Student}, gerando anotações automatizadas (\textit{pseudo-labeling}); e modelos YOLO11~\cite{yolo11_khanam2024}, que atuam como Alunos especialistas, otimizados para inferência rápida na linha de produção. A contribuição central deste trabalho concentra-se no segundo estágio --- o segmentador especialista \textit{Teacher-Student}: o classificador DeepStoneAI é reaproveitado como porta de entrada do domínio, e é a especialização da segmentação por litologia, e não a classificação em si, o objeto desta investigação.
...
No ambiente de manufatura contínua, em que prevalecem dispositivos de recursos computacionais restritos --- a chamada computação na borda (\textit{Edge Computing}) ---, a velocidade de inferência é prioritária. O modelo Aluno adotado é a arquitetura YOLO (\textit{You Only Look Once})~\cite{yolo_redmon2016}, em sua versão 11~\cite{yolo11_khanam2024}. Em uma única passagem pela rede, a YOLO executa tanto a extração de características --- na porção da rede chamada \textit{backbone} --- quanto a predição das máscaras --- na chamada \textit{head} ---, atingindo altas taxas de quadros por segundo (FPS). Essa eficiência viabiliza a inferência em tempo real na linha de produção, requisito central da aplicação industrial proposta.
...
\subsection{Calibração do SAM por Litologia}

A injeção de conhecimento de domínio no Professor é operacionalizada por um arquivo de configuração (\texttt{rock\_prompts.json}) que associa, a cada litologia, um conjunto de \textit{prompts} de anomalia e um limiar de confiança próprio para cada um deles. A motivação é direta: um limiar adequado para realçar fissuras em um mármore branco tende a super-segmentar a textura de um granito escuro, produzindo falsos positivos. Calibrar \textit{prompts} e limiares por tipo de rocha é, portanto, o mecanismo concreto que converte um modelo genérico em um segmentador especializado no domínio de cada litologia.

Os \textit{prompts} são expressos em linguagem natural e interpretados pelo CLIP~\cite{clip_radford2021}, que aproxima o vetor textual das regiões visualmente similares. Como o CLIP foi treinado a partir de legendas, e não de palavras isoladas, \textit{prompts} contextuais (por exemplo, \textit{crack on a stone surface}) tendem a desambiguar melhor o alvo do que termos isolados (\textit{crack}), que podem evocar contextos alheios ao domínio. A sensibilidade dos resultados à formulação dos \textit{prompts} é, ela própria, uma variável de análise prevista para as etapas quantitativas.

Em vez de ajustar 45 configurações independentes a partir do zero, as litologias foram organizadas em grupos por cor e textura, que compartilham estratégias de \textit{prompt} e faixas de limiar semelhantes, com refinamentos individuais quando necessário (Tabela~\ref{tab:calibracao}). Rochas claras, por exemplo, empregam o \textit{prompt} \textit{dark patches} para oxidações --- ausente nas escuras, que em contrapartida usam \textit{light spot} --- e rochas escuras exigem limiares mais baixos para que anomalias sutis não passem despercebidas. A configuração padrão de referência, usada como ponto de partida, adota limiares de 0,1 para \textit{crack}, 0,007 para \textit{vein} e 0,3 para \textit{stain}.
...
Amarelas / bege & Grupo intermediário; incluem \textit{dark patches} \\
...
A seleção da imagem representativa de cada litologia, sobre a qual a calibração é conduzida, é feita \textbf{manualmente}. Trata-se de uma decisão metodológica deliberada: empregar o próprio SAM para escolher a imagem de calibração configuraria raciocínio circular, enviesando a avaliação do modelo a partir de uma amostra que ele mesmo selecionou. Para cada litologia, ajusta-se a configuração e executa-se a inferência de forma iterativa, inspecionando as máscaras resultantes até que a segmentação seja considerada satisfatória; as anotações finais são exportadas como polígonos no formato de segmentação do YOLO (coordenadas normalizadas).
